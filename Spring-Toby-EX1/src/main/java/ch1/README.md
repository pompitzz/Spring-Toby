...
> 즉 스프링은 IoC컨테이너일 뿐만아니라, 고전적인 싱글톤 패턴의 한계를 대신해서 싱글톤을 만들고 관리해주는 싱글톤 레지스트리이다.
...
- **런타임 의존관계는 설계 모델 관점의 의존관계와는 다르다.**

> - 인터페이스를 통해 설계 시점에 느슨한 의존관계를 갖는 경우 UserDao의 오브젝트가 런타임시에 사용할 오브젝트가 어떤 클래스로 만든것인지 미리 알 수가 없다.
