# Chapter3. 템플릿
- 개방 폐쇄 원칙은 확장에는 열려있고 변경에는 굳게 닫혀있는 객체지향의 핵심 설계 원칙이다.
- 개방 폐쇄 원칙을 적극 도입하면 각각 다른 목적과 다른 이유에 의해 다른 시점에 독립적으로 변경될 수 있는 효율적인 구조를 만들어 준다.
...
- 그렇다고 테스트를 통해 DAO 메서드당 리소스를 반납하는지 테스트하기에는 커넥션이나 Statement의 구현 클래스를 구현해 테스트하여야하므로 쉽지 않다.

#### 이런 코드를 효과적으로 다룰 수 있는 방법은 없을까?

### 3.2.2 분리와 재사용을 이용한 디자인 패턴 적용
- 메서드 추출을 할 수 있겠지만 sql 자체를 분리시킬 수 없어 재사용이 불가능해진다.
- 이럴때 템플릿 메서드 패턴을 적용하면 효율적으로 분리할 수 있다.
...
- 개방 폐쇄 원칙을 잘 지켜 변화에는 닫혀있고 확장에는 열려있도록하였지만 이 방식은 기능이 추가될 때 마다 클래스를 만들어야 하는 큰 단점이 존재한다.
...
- 보통의 DI는 템플릿에 인스턴스 변수를 만들고 그 인스턴스를 주입해주지만 현재는 메서드단위로 DI가 일어난다는 것이 특징이다.
...
> - 단순한 전략패턴으로만 보기엔 독특한 특징이 많으므로 고유한 디자인 패턴으로 기억하는것이 편리하다.
...
### 3.5.3 템플릿 / 콜백 응용
- 템플릿 / 콜백 패턴은 사실 스프링에서만 사용할 수 있다거나 스프링만이 제공해주는 독점적인 기술은 아니다.
...
- 스프링은 기본적으로 OCP를 지키고, 전략 패턴과 DI를 바탕에 깔고 있으니 원한다면 확장해서 편리하게 사용할 수 있다.
...
- [Calculator](./step14/Calculator.java)를 확인해보면 각 계산 메서드는 단지 람다식을 정의해서 콜백으로 넘기는 것으로 매우 간단하고 깔끔하게 구현이 가능해졌다!!
...
### 테스트 보안
- 테스트를 꼼꼼히 작성해야 한다.
- 유저 전체를 조회할 때에도 유저가 없을때, 아이디에 이상한 값이 들어갈때 등등에 대한 처리도 해주어야 유지보수가 쉬운 서비스를 만들 수 있다.
