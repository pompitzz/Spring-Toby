...
- 동적으로 부가기능을 부여하기 떄문에 데코레이터 패턴이라고 할 수 있을 것이다.
...
> 이런식으로 만약 참조는 가지고 있지만 끝가지 사용하ㄱ지 않거나, 많은 작업이 진행된 후에 사용되는 경우라면, 이런식으로 프록시를 통해 생성을 최대한 늦춤으로서 얻는 장점은 많이 존재한다.
...
- 다이내픽 프록시는 프록시 팩토리에 의해 런타임 시 다이내믹하게 만들어지는 오브젝트이다.
- 다이내믹 프록시 오브젝트는 타깃의 인터페이스와 같은 타입으로 만들어 진다.
- 클라이언트는 다이내픽 프록시 오브젝트를 타깃 인터페이스를 통해 사용할 수 있다.
- 이 덕분에 프록시를 만들 때 인터페이스를 모두 구현하지 않고 프록시 팩토리에게 인터페이스 정보만 제공해주면 그에 맞는 프록시를 만들어 준다.
- 다이내픽 프록시가 인터페이스 구현 클래스의 오브젝트는 만들어주지만, 프록시로서의 부가 기능은 직접 개발자가 작성해야 한다.
...
- 순서대로 클래스로더, 구현 인터페이스, 부가기능을 담은 InvocationHandler를 파라미터로 넘겨주면 다이내픽 프록시를 생성해준다.
...
- 이제 UserServiceTx를 다이내픽 프록시 방식으로 변경해보자.
...
- 하지만 다애내믹 프록시는 스프링 빈으로 등록할 수 없다. 다이내믹 프록시는 리플렉션을 통해 오브젝트를 생성할 수 없다. 왜냐하면 해당 클래스 자체도 내부적으로 다이내믹하게 정의되기 때문에 스프링이 알 수 있는 방법이 없다.
- 다이내픽 프로시는 Proxy.newProxyInstance를 통해 만들기 때문이다.
...
- 팩토리빈 인터페이스를 구현한 클래스를 스프링의 빈으로 등록하면 팩토리 ㅣㅂㄴ으로 동작한다.
- 이에 대한 학습테스트를 작성해보자.
...
- 이런방식으로 다이내픽 프록시를 만들어주는 팩토리 빈을 생성해보자.
...
- 팩토리 빈이 만드는 다이내픽 프록시는 Object로 구현되었기 때문에 타깃의 종류에 제한이 없게 된다.
...
- 다이내픽 프록시를 생성해주는 팩토리 빈을 사용하는 방법은 여러 장점이 존재한다.
...
- ProxyFactoryBean을 이용하여 이젠에 만들었던 Hello Proxy를 테스트해보자.
...
```
- ProxyFactoryBean에 타겟과 어드바이스를 추가하면 이전에 만든 다이내픽 프록시와 동일하게 동작한다.
...
- **바로 이 점이 JDK의 다이내픽 프록시를 직접 사용하는 코드와 스프링이 제공해주는 프록시 추상화 기능인 ProxyFactoryBean을 사용하는 코드의 가장 큰 차이점이자 ProxyFactoryBean의 장점이다.**
- ProxyFactoryBean은 작은 단위의 템플릿 / 콜백 구조를 응용해서 적용되어 템플릿 역할을 하는 MethodInvocation을 싱글톤으로 두고 공유할 수 있다.
- 마지 SQL 파라미터 정보에 종속되지 않은 JdbcTemplate이기 때문에 JdbcTemplate를 싱글톤으로 여러 DAO가 공유할 수 있는 것과 동일하다.
- ProxyFactoryBean은 advice를 추가할 때 addAdvice를 호출한다. 즉 여러개의 Advice를 추가할 수 있다는 것이다. 이 또한 이전의 다이내픽 프록시의 단점을 보완해준다.
...
- **MethodInterceptor처럼 타깃 오브젝트에 적용하는 부가기능을 담은 오브젝트를 스프링에서는 어드베이스라고 부른다.**
- 그리고 다이내픽 프록시에서는 구현할 인터페이스를 제공해주었지만 ProxyFactoryBean에서는 제공할 필요가 없다.
...
- 재사용 가능한 기능을 만들어 두고 바뀌는 부분만 외부에서 주입해서 이를 작엄 흐름 중에 사용하도록 하는 전형적인 템플릿 / 콜백 구조이다.
...
- 그 덕분에 여러 프록시가 공유해서 사용할 수 있고, 또 구체적인 부가기능 바식이나 메서드 선정 알고리즘이 바뀌면 구현 클래스만 바꿔서 설정에 넣어주면 될 것이다.
...
- 콜백을 호출하므로 타깃정보가 필요없어지며 해당 메서드가 호출된다는 것은 타깃으로 지정된 것이므로 따로 다른 설정을할 필요 없이 바로 트랜잭션을 적용하면 된다.
- 다이내믹 프록시는 예외를 Method에서 따로 변환하여 던져주지만 MethodInvocation은 포장하지 않고 바로 던져주므로 그대로 전달이 가능하다.
...
- 그러므로 UserService이외의 새로운 비즈니스 로직을 담은 서비스 클래스가 생겨도 해당 transactionAdvice, transactionAdvisor, transactionPointCut은 재사용이 가능해졌다.
...
- 이렇게 분리해낸 트랜잭션 코드는 투가명한 부가기능 형태로 제공돼야 한다.
...
- 변하지 않는 타깃으로의 위임과 부가기능 적용 여부 판단이라는 부분은 코드 생성 기법을 이용하는 다이내믹 프록싱 기술에 맡기고, 변하는 부가기능 코드는 별도로 만들어서 다이내믹 프록시 생성 팩토리에 DI로 제공하는 방법을 사용한 것이다. 이를 통해 변하는 로직과 변하지 않는 기계적인 코드를 잘 분리해낸 셈이다.
...
- 스프링은 빈 추러기기가 빈으로 등록되어 있다면 빈 오브젝트가 생성될 때마다 빈 추러기에 보내서 후처리 작업을 요청한다.
- 빈 후처리기는 빈 오브젝트의 프로퍼티를 강제로 수정할 수도 있고 별도의 초기화 작업을 수행할 수도있다.
- 심지어 만들어진 빈 오브젝트 자체를 바꿔치기할 수도 있다.
- 따라서 스프링아 설정을 참고해서 만든 오브젝트가 아닌 다른 오브젝트를 빈을 등록시키는 것이 가능하다.
...
- 이렇게 포인트컷 표현식은 인터페이스, 슈퍼클래스의 타입도 인식해준다.
...
- 결국 비즈니스 로직 코드는 트랜잭션과 같은 성격이 다른 코드로부터 자유로워 졌고 독리벅으로 로직을 검증하는 고립된 단위 테스트를 만들 수 있게 되었다.
...
- 그러하여 다이내픽 프록시와 같은 프록시 기술을 추상화한 스프링 프록시 팩토리 빈을 이용하여 다이내믹 프록시 생성 방법에 DI를 도입했다. 내부적으로 템플릿 / 콜백 패턴을 활용하는 스프링의 프록시 팩토리 빈 덕분에 부가기능을 담은 어드바이스와 포인트컷은 분리될 수 있었고 여러 프록시에서 공유해서 사용할 수 있게 됐다.

#### 자동 프록시 생성 방법과 포인트 컷
- 추상화된 프록시 팩토리 빈을 사용하였지만 각 서비스마다 프록시 팩토리 빈을 만들어줘야하는 부담이 있었다.
- 그러하여 빈 생성 추러기 기법을 활용하여 컨테이너 초기화 시점에 자동으로 빈들을 검색하여 포인트 컷 조건에부합하면 프록시를 생성하도록 하였다.
...
#### AOP: 애스팩트 지향 프로그래밍
- 트랜잭션과 같은 부가기능을 어떻게 모듈화할 것인지 연구해온 사람들은, 이 부가기능 모듈화 작업은 기존의 객체지향 설계 패러다임과는 구분되는 새로운 특성이 이싿고 생각했다.
...
- 프록시로 만들어 DI로 연결된 빈 사이에 적용해 타깃의 메서드 호출과정에 참여하여 부가기능을 제공해주도록 만들었따.
...
- 여기서 어드바이스를 제외한 모든 것은 스프링이 제공하눈 클래스를 빈으로 등록하여 사용한다.
